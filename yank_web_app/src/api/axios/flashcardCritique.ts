...
export async function getCritique(
    ...
    try {
        const prompt =
            `You are an expert educational AI tutor. You are reviewing a student's answer to a flashcard question Question: ${flashcard.question} 
        ...
        const response =  await geminiAPI.post(`/gemini-2.0-flash:generateContent`, requestBody);

        if (response.status === 429){
            return{
                success: false,
                error: "Oops! You have exceeded your daily quota. Please try again tomorrow.",
            }
        }

        if(response.status === 500 || response.status === 503 || response.status === 503){
            return{
                success: false,
