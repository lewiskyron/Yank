...
export default function GoogleSigninButton({ text }: { text: string }) {
	...
	const handleGoogleSignUp: () => Promise<void> = async () => {
	...
	return (
		<form action="">
			{/* Conditionally render AlertError and AlertSuccess components */}
			{errorMessage && <AlertError errors={[errorMessage]} />}
			<button
				type="button"
				onClick={handleGoogleSignUp}
				className="border-stroke bg-gray-2 dark:border-dark-3 dark:bg-dark-2 flex w-full items-center justify-center gap-3.5 rounded-lg border p-[15px] font-medium hover:bg-opacity-50 dark:hover:bg-opacity-50"
			>
				<span>
					<svg
						width="20"
